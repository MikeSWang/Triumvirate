...
static double calc_lowring_kr_pivot(
  ...
  if (arg != arg_int) {kr_c *= exp((arg - arg_int) * L/N);}
...
/**
 * Perform the cosmological Hankel transform (spherical Bessel transform).
 *
 * @param[in] ell Order of the transform.
 * @param[in] m Power-law index of the source term.
 * @param[in] N Sampling number.
 ...
 */
int cosmo_hankel_transform(
  int ell, int m, int N, double* k, double* pk, double* r, double* xi
) {  // !!! check power-law indices; rename arguments
  ...
  forward_hankel_transform(N, ell + 1./2, 0., 1., 1, k, r, a, b, NULL);
...
int transform_power_spec_to_corr_func(
  ...
  cosmo_hankel_transform(0, 2, N, k, pk, r, xi);
...
int transform_corr_func_to_power_spec(
  ...
  cosmo_hankel_transform(0, 2, N, r, xi, k, pk);
