 ...
 * Collection of tools.
 *
 * This collection includes calculators for the 'reduced' spherical
 * harmonics with desired normalisation, and storage of calculations
 * performed in configuration/Fourier space.
 *
 */
class ToolClass {
 public:
	 ...
	 * @returns ylm Value of the reduced spherical harmonic.
	 */
	static std::complex<double> calc_reduced_spherical_harmonic(int ell, int m, double* pos) {
		...
		/// Use parity (-1)^((m - |m|)/2) conjugate for m < 0.
		if (m < 0) {ylm = pow(-1., m) * std::conj(ylm);}

		/// Renormalise to the reduced form.
		ylm *= sqrt(4.*M_PI / (2.*double(ell) + 1.));
