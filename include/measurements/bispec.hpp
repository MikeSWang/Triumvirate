            ...
            for (int j = 0; j < params.nmesh[1]; j++) {
              for (int k = 0; k < params.nmesh[2]; k++) {
                ...
                std::complex<double> shotnoise_grid(
									three_pt_holder[coord_flat][0],
									three_pt_holder[coord_flat][1]
								);
                double j1 = sj1.eval(kmag_a * rmag);
                double j2 = sj2.eval(kmag_b * rmag);

                shotnoise_sum +=
									j1 * j2 * ylm_a[coord_flat] * ylm_b[coord_flat]
									* shotnoise_grid;
              }
            }
          }
        ...
        for (int i = 0; i < params.nmesh[0]; i++) {
          for (int j = 0; j < params.nmesh[1]; j++) {
            for (int k = 0; k < params.nmesh[2]; k++) {
              ...
              std::complex<double> shotnoise_grid(
								three_pt_holder[coord_flat][0], three_pt_holder[coord_flat][1]
							);
              double j1 = sj1.eval(kmag_a * rmag);
              double j2 = sj2.eval(kmag_b * rmag);
 ...
 * @returns Exit status.
 */
int calc_3pt_corr_func_window_mpi(
	ParticleCatalogue& particles_rand,
	LineOfSight* los_rand,
	ParameterSet& params,
	double alpha,
 ...
 * Calculate three-point correlation function window for three-point
 * correlation function from random catalogues and save the results.
 * This function uses logarithmic binning except on the smallest scales,
 * where discretionary bins are used.
 *
 * @param particles_rand (Reference to) the random-source particle container.
 * @param los_rand Random-source particle lines of sight.
 * @param params (Reference to) the input parameter set.
 ...
 * @returns Exit status.
 */
int calc_3pt_corr_func_window_in_log_bins(
	ParticleCatalogue& particles_rand,
	LineOfSight* los_rand,
	ParameterSet& params,
	double alpha,
 ...
 * @returns Exit status.
 */
int calc_3pt_corr_func_window_for_wide_angle(
	ParticleCatalogue& particles_rand,
	LineOfSight* los_rand,
	ParameterSet& params,
	double alpha,
	...
	for (int m1_ = - params.ell1; m1_ <= params.ell1; m1_++) {
		for (int m2_ = - params.ell2; m2_ <= params.ell2; m2_++) {
			...
			for (int M_ = - params.ELL; M_ <= params.ELL; M_++) {
				...
				/// Calculate x^{-i-j} G_{LM}.  See `calc_3pt_corr_func`.s
				DensityField<ParticleCatalogue> dn_LM(params);
				dn_LM.calc_ylm_weighted_mean_density(
					particles_rand, los_rand, alpha, params.ELL, M_
				);
